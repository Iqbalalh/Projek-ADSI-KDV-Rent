...
<ul class="nav nav-pills flex-column mb-sm-auto mb-0 align-items-center align-items-sm-start" id="menu">
  ...
  <br>
  <form action="" method="post" id="konfirmasiForm">
    <div class="table-responsive">
      <table class="table align-middle">
        ...
        <tbody>
          <?php while ($result = mysqli_fetch_assoc($sql)) { ?>
            <tr>
              <td>
                <input type="checkbox" name="konfirmasi[]" value="<?= $result['id']; ?>" <?= ($result['konfirmasi'] == 'ya') ? 'checked' : ''; ?>>
              </td>
              <td><?= $result['nama']; ?></td>
      ...
      <button type="button" id="konfirmasiButton" class="btn btn-primary float-end">Konfirmasi</button>
    </div>
  </form>
</div>
...
<script>
  $(document).ready(function() {
    $("#konfirmasiButton").click(function() {
      var konfirmasiData = $("#konfirmasiForm").serialize();
      $.ajax({
